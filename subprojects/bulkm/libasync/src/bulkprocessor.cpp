...
BulkProcessor::BulkProcessor(const std::string& rootLogDirectory) :
    m_needExit{false}
{
    auto pThread = std::make_unique<std::thread>(&BulkProcessor::printerFileThreadFunc, this, rootLogDirectory);
    m_threads.push_back(std::move(pThread));

    pThread = std::make_unique<std::thread>(&BulkProcessor::printerFileThreadFunc, this, rootLogDirectory);
    m_threads.push_back(std::move(pThread));
...
{
    auto printerFile = std::make_unique<Bulkm::BulkPrinterFile>(rootLogDirectory);
    std::queue<Bulkm::ICommandStorage::Ptr> que;
